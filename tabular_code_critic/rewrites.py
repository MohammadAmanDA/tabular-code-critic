...
class FilterAggregatePatternFinder(ast.NodeVisitor):
    ...
    def _find_count_pattern(self, node: ast.Module) -> None:
        """
        Pattern:
            count = 0
            for _, row in df.iterrows():
        ...
        for stmt in node.body:
            ...
            if not isinstance(aug.target, ast.Name):
                continue
            if not (isinstance(aug.value, ast.Constant) and aug.value.value == 1):
                continue
...
def suggest_vectorized_count(code_str: str) -> Optional[str]:
    tree = ast.parse(code_str)
    finder = FilterAggregatePatternFinder()
    finder.visit(tree)

    c = finder.count_candidate
    if c is None:
        return None

    return f'{c["acc_name"]} = ({c["cond_expr"]}).sum()'
